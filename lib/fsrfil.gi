...
#############################################################################
##
#F  FSRFIL( <K>, <clist>, <mlist>  )  . . . .  create an FSRFIL object 	# len 3
#F  FSRFIL( <K>, <fieldpol>, <clist>, <mlist> ,)  . . . .  create an FSRFIL object 	# len 4
#F  LFSR( <K>, <poly> )  . . . . . . . . . .  create an LFSR object 	# len 2 B
#F  LFSR( <K>, <poly>, <B> )  . . . . . . . . . .  create an LFSR object 	# len 3 B
#F  LFSR( <K>, <fieldpol>, <poly>)					# len 3 B
#F  LFSR( <K>, <fieldpol>, <poly>, <B>)					# len 4 B

InstallGlobalFunction( FSRFIL,  function(arg)
...
if Length(arg)=2 then 
	...
	if  IsField(arg[1]) and IsPolynomial(arg[2]) and IsFFECollection(arg[3]) and IsList(arg[4]) then 	
...
# new FSRFIL :) 
	...
	fsrfil := Objectify(NewType(fam, IsFSRRep),   rec(init:=st, state:= st, numsteps := 0, basis := CanonicalBasis(F)));
#	fsrfil := Objectify(NewType(fam, IsFSRRep),   rec(state:= st, numsteps := 0, basis := CanonicalBasis(F)));
#  without init works just fine, but rather not since there are so many methods maybe one of em uses init 
...
InstallMethod( ConstTermOfFSRFIL, "const term of the multivariate polynomial",  [IsFSRFIL], function(x)
