...
##  three cases, it is computationally more demanding. <P/>
##  Elxample below shows a LFSR using firstly a reducible feedback polynomial
##  and then a primitive feedback polynomial. 
##  <Example>
##  <![CDATA[
...
##  255
##  false
##  gap> l := y^4 + y^3 + y + Z(2^4);; test := LFSR(K, f, l, B);
##  < empty LFSR over GF(2^4) given by FeedbackPoly = y^4+y^3+y+Z(2^4) >
##  gap> Period(test); IsMaxSeqLFSR(test);
##  65535
##  true
