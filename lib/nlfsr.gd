...
##  finite field. <P/>
##  NOTE: before creating the <C>NLFSR</C>, we must always create the indeterminates to be used for the 
##  feedback using <C>ChooseField</C> function call!!! please see example below <P/>
##  <P/>
##  Inputs:
##  <List>
...
##  <Item> <A>tap</A> - optional parameter: the output tap (must be a positive integer or a 
##  list of positive integers) and will be changed to the default S_0 if the specified integer is 
##  out of <C>NLFSR</C>range.</Item>
##  </List>
##  NOTE: <A>clist</A>  and <A>mlist</A> must be of same length, all elements in <A>clist</A> must 
##  belong to the underlying field. Monomials in <A>mlist</A> 
##  must not include any indeterminates that are out of range specified by <A>len</A>: stages of 
##  <C>NLFSR</C>  are represented by indeterminants and the feedback is not allowed to use 
##  a stage that doesnt exist. A second constraint on <A>mlist</A> requires that it must contain at
...
##  a reducible poynomial), an error message appears and the function returns <C>fail</C>.
##  <Example>
##  <![CDATA[
