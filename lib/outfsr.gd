...
##
##  <Description>
##  Equivalent to PrintAll , but it writes to an output stream. NOTE: The basis switch must be present and if <E>true</E>, the currently set basis of the <A>fsr</A> is used. 
##  </Description>
##  </ManSection>
##  <#/GAPDoc>
...
#############################################################################
##
#F  WriteSequenceFSR ( <output>, <lfsr>, <sequence> ) . . . . write output sequence to file
##
...
##  <Func Name="WriteTEXSequenceByGenerator" Arg="output, fsr, sequence, strGen, gen"/>
##
##  <Description>
##  <C>WriteSequenceFSR</C> writes the sequence generated by some version of RunFSR(lfsr)  to *.txt file, 
##  with addition of separating sequences from different taps and writing them in currently set basis of the <A>fsr</A>.
##  <P/>
##  <C>WriteTBSequenceFSR</C> is a version of <C>WriteSequenceFSR</C>  intended for testbenching purposes: the generated sequence is written to *.txt file, 
##  with sequences from different taps separated into <E>columns</E> separated by "\t". Again the currently set basis of the <A>fsr</A> is used. 
##  The order of columns is determined by  <C>OutputTap(<A>fsr</A>)</C>. 
##  <P/>
##  <C>WriteTEXSequenceByGenerator</C> is a *.tex version of <C>WriteSequenceFSR</C>  but allows to write the sequence elements as powers of a chosen generator <A>gen</A>.
##  Generator <A>gen</A> is used to get the exponents of the elements, and the elements themselfs are printed as $\<A>strGen</A>^{exponent}$, where 
##  strGen must be a string representing a greek letter in *.tex, for example <A>strGen</A> "alpha" will give <M>\alpha</M>. 
##  </Description>
##  </ManSection>
##  <#/GAPDoc>
##
...
#############################################################################
##
#F  WriteRunFSR( <output>, <lfsr>, <ist>, <numsteps> ) . . . . . . . . write elm to file
##
##  <#GAPDoc Label="WriteRunFSR">
##  <ManSection>
##  <Func Name="WriteRunFSR" Arg="output, lfsr, ist, numsteps"/>
##  <Func Name="WriteNonlinRunFSR" Arg="output, lfsr, ist, numsteps"/>
##
##  <Description>
##  <C>WriteRunFSR</C> is an output to *.txt version of RunFSR(<A>fsr</A>, <A>ist</A>, <A>num</A>), with addition of separating sequences from different taps and writing them in currently set basis of the <A>fsr</A>. 
##  Before the run begins and after loading, the <C>WriteAllFSR(output,  x, true)</C> is called to record the FSR being used. When the run is finished, <C>WriteSequenceFSR</C>
##  is called to record the output sequence in compact version. <C>WriteRunFSR</C> returns the sequence generated by this run.
##  <P/>
##  <C>WriteNonlinRunFSR</C> is an output to *.txt version of RunFSR(<A>fsr</A>, <A>ist</A>, <A>elmvec</A>). <C>WriteNonlinRunFSR</C> returns a sequence generated by this run, 
##  however, the length of returned sequence is Length(<A>elmvec</A>)+1, because the first element of the output sequence is the element that was loaded with <A>ist</A>. 
##  </Description>
##  </ManSection>
##  <#/GAPDoc>
...
##
##  <Description>
##  <C>WriteTEXRunFSR</C> is an output to *.tex version of RunFSR(<A>fsr</A>, <A>ist</A>, <A>num</A>), which writes a table that can be included dircetly (except for the label). 
##  Rows of the table represent the steps of the FSR and include the state of the FSR and the elements from stages specfied by outputTap, that is the sequence outputs at this step.
...
##  is called to record the output sequence in compact version. <C>WriteTEXRunFSR</C> returns the sequence generated by this run.
##  <P/>
##  <C>WriteTEXNonlinRunFSR</C> TO DO 
##  <P/>
##  <C>WriteTEXRunFSRByGenerator</C>  is a *.tex version of <C>WriteTEXRunFSR</C>  but instead of using the currently set basis of the <A>fsr</A>, the table entries 
##  are printed as  powers of a chosen generator <A>gen</A>.
##  Generator <A>gen</A> is used to get the exponents of the elements, and the elements themselfs are printed as $\<A>strGen</A>^{exponent}$, where 
##  strGen must be a string representing a greek letter in *.tex, for example <A>strGen</A> "alpha" will give <M>\alpha</M>. 
##  </Description>
##  </ManSection>
...
#############################################################################
##
#F  WriteTEXElementTableByGenerator( <output>, <F>, <B>, <strGen>, <gen> ) . . . . . . . . write elms table with pwrs of generators to file
##
...
##
##  <Description>
##  <C>WriteTEXElementTableByGenerator</C> provides the context information for  <C>WriteTEXSequenceByGenerator</C> and   <C>WriteTEXRunFSRByGenerator</C>. 
##  Its output is a *.tex file with a table containing the elements of <A>F</A> represented in basis <A>B</A> and their representation as powers of a chosen generator <A>gen</A>, 
##  printed as $\<A>strGen</A>^{exponent}$, where by the greek letter passed to the function as a string <A>strGen</A>. There is an extra table column containing the order of each element. 
##  <P/>
##  The output file contains additional information: defining polynomial of <A>F</A>, basis elements of  <A>B</A> as powers of generator <A>gen</A>, and information whether or not 
##  <A>gen</A> is a root of the defining polynomial.
##  </Description>
##  </ManSection>
##  <#/GAPDoc>
