...
InstallGlobalFunction( TikzSimple_LFSR, function(output, x)
local  n, nzt, i, len; 
...
InstallGlobalFunction( TikzSimple_nLFSR, function(output, x)
local  n, nzt, i, len; 
...
if (IsOutputStream( output )) then
	...
	#	Print(nzt);
		...
		AppendTo(output,  OutputTap(x)[Length(OutputTap(x))], "}{\n"); 
		AppendTo(output, "\\pgfmathtruncatemacro{\\x}{\\n-\\t-1};\n \\draw [->,thick] (\\x+1.5,0)-- ++(0,-\\s) --++(1+\\t,0)  node [right] {$s_{\\t}$};\n }\n");
		AppendTo(output, "\\end{tikzpicture}\n \\caption{LFSR with feedback polynomial $f(x)=", CharPoly(x),"$ and additional ``nonlinear'' input}\\label{LABEL}\n \\end{figure}\n \\end{center}\n");
...
InstallGlobalFunction( TikzComplex_LFSR, function(output, x)
...
InstallGlobalFunction( TikzComplex_nLFSR, function(output, x)
local  n, nzt, i, len, simple, complex, idx; 
...
InstallGlobalFunction( Tikz_LFSR, function(output, x)

if IsPrimeField(DefaultField(FeedbackVec(x))) then TikzSimple_LFSR(output, x);
else TikzComplex_LFSR(output, x);
fi;
...
InstallGlobalFunction( Tikz_nLFSR, function(output, x)

if IsPrimeField(DefaultField(FeedbackVec(x))) then TikzSimple_nLFSR(output, x);
else TikzComplex_nLFSR(output, x);
fi;
